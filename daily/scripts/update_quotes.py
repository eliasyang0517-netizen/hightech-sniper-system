...
def sina_quotes(stocks):
    """Sina real-time quotes (hq.sinajs.cn) as a resilient extra source.

    Requires a Referer header or it returns 403. Only core price/OHLC/volume
    fields are mapped; PE/PB/market-cap are filled by the other sources.
    """
    symbol_to_code = {sina_symbol(stock["code"]): str(stock["code"]) for stock in stocks}
    symbols = list(symbol_to_code)
    result = {}
    for offset in range(0, len(symbols), 80):
        ...
        for symbol, payload in re.findall(r'var hq_str_([^=]+)="([^"]*)"', body):
            original_code = symbol_to_code.get(symbol.lower()) or symbol_to_code.get(symbol)
            if not original_code:
                continue
            is_hk = original_code.upper().endswith(".HK")
            quote = parse_sina_fields(payload.split(","), is_hk=is_hk)
            if quote.get("price"):
                quote["provider"] = "新浪实时"
                result[original_code] = quote
    return result
...
SOURCE_PRIORITY = ["腾讯行情", "新浪实时", "AKShare（东方财富行情）"]


def merge_quotes(sources, now):
    """Merge multiple quote sources with pairwise disagreement checks.

    `sources` maps a provider name to its quote dict (or None). The highest
    priority available source becomes primary; missing fields are back-filled
    from the others. When two or more sources disagree beyond the limit the
    quote is rejected rather than written.
    """
    available = {name: q for name, q in sources.items() if q and q.get("price")}
    if not available:
        return None, "无可用行情源"
    ...
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            diff = relative_difference_pct(prices[names[i]], prices[names[j]])
            if diff is not None:
                max_diff = max(max_diff, diff)
    if len(names) >= 2 and max_diff > SOURCE_DISAGREEMENT_LIMIT_PCT:
        return None, f"多源价格偏差 {max_diff:.2f}% 超过阈值"

    primary_name = next((n for n in SOURCE_PRIORITY if n in available), names[0])
    primary = dict(available[primary_name])
    for name in names:
    ...
    if len(names) >= 2:
        validation_status = "dual-source"
        validation_label = "三源一致" if len(names) == 3 else "双源一致"
    else:
        validation_status = "single-source"
        validation_label = "单源有效"
    primary.update(
        {
            "provider": " + ".join(names),
            ...
            "sourcePrices": prices,
            "sourceDifferencePct": round(max_diff, 4) if len(names) >= 2 else None,
...
def main():
    ...
    try:
        quotes_sina = sina_quotes(stocks)
    except Exception as exc:
        quotes_sina = {}
        failures.append(f"新浪实时行情失败: {exc}")
    ...
    for stock in stocks:
        ...
        quote, rejection_reason = merge_quotes(
            {
                "腾讯行情": quotes_tencent.get(code),
                "新浪实时": quotes_sina.get(code),
                "AKShare（东方财富行情）": lookup.get(key),
            },
            now,
        )
        if rejection_reason:
        ...
        market.update(
            {
                ...
                if quote.get("floatMarketCapLocalYi")
                else None,
    ...
    meta.update(
        {
            "quoteProvider": "腾讯行情；新浪实时；AKShare/东方财富可用时进行三源校验",
