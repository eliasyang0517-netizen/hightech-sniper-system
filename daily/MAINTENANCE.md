...
- `scripts/update_quotes.py`：更新 A 股、港股行情，执行三源（腾讯 + 新浪 + 东方财富）/滞后/异常跳变/市值单位校验，并生成自动市场分析。
...
- 行情：工作日北京时间 16:10（收盘后）触发一次。
- 公告和新闻：工作日北京时间 18:45 触发一次。
- GitHub 的定时任务可能延迟数分钟，不是交易所级实时行情。

## 数据来源

- 行情：腾讯公开行情与新浪实时行情为主源；AKShare 封装的东方财富行情可用时做三源价格校验。三者价格两两偏差超过 3% 时拒绝写入，避免错价污染。
- A 股公告：巨潮资讯。
- 新闻：东方财富。
- 港股市值按动态 HKD/CNY 汇率折算为亿元人民币。脚本依次尝试 Frankfurter、Open Exchange Rate API 和 ExchangeRate API；全部不可用时才使用 `0.92`，页面会明确标为“后备”。

行情任务还会检查：多源（双源或三源）价格偏差、行情是否滞后、相对旧价格是否异常跳变，以及“市值÷股价”隐含股本是否出现异常变化。异常价格会拒绝写入；市值单位异常时只保留旧市值，不影响有效股价更新。
